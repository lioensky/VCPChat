# VCPChat — 分布式 AI 原生全栈引擎

[English](vcpchatREADME_en.md) | [日本語](vcpchatREADME_jp.md) | [Русский](vcpchatREADME_ru.md)

**VCPChat 不再仅仅是一个聊天客户端，它是一个打通 AI-UI-App 边界的分布式全栈底层引擎。**

通过对前端 IPC 和后端 API 的深度虚拟化封装，VCPChat 构建了一个超越传统交互范式的 AI 原生运行环境。
...
*   **极致封装**: 凭借底层的高级流式渲染、分布式通讯和全栈插件系统，开发者现在仅需 **30 行代码** 即可构建一个功能完备的 `VChatMini`。
*   **降维打击**: 相比 VCPChat 自身 30 万行代码的底层积淀，这种高度抽象让复杂的 AI 应用开发变得如同拼积木般简单。
*   **全栈集成**: 无论是 HIFI 音频引擎、流式 HTML 渲染，还是跨节点的分布式文件追踪，所有能力都已通过虚拟化接口（vcpAPI）对开发者透明开放。
...
VCPChat 不仅仅是一个聊天界面，它是强大的 VCP 后端生态（VCPToolBox）的“眼睛”和“画板”。两者深度结合，旨在释放 AI 的无限潜能：

*   **高级能力渲染**: VCPChat 专为渲染 VCP 协议的复杂输出而设计。无论是 AI 主动记录和反思的“日记”，还是多媒体内容，都能以最直观的方式呈现。VCPChat 的 Agent 输出气泡拥有近乎怪物级的渲染能力，可以渲染几乎所有主流动画和文档。
*   **重型异步任务**: 后端可以执行如视频生成、数据分析等耗时很长的重型任务。AI 发起任务后可立即响应用户，任务完成后，VCPChat 会通过后端推送，将结果（如生成的视频）实时展现在对话流中，整个过程流畅不卡顿。
*   **Agent 群体智能**: 后端支持多个 AI Agent 协同工作，甚至实现 Agent 主动分配任务给子 Agent。VCPChat 的群聊模式和清晰的发言标记系统，为这种高级的“AI 女仆团”协作流提供了完美的交互界面。
*   **丰富的多媒体交互**: AI 可以通过后端调用 VCP 工具，在对话中发送表情包、播放音乐、展示视频。VCPChat 拥有强大的多媒体渲染引擎和高级的窗口气泡动画、流式动画，确保了极致的视听体验。
...
### 📝 VCP Scriptorium — 共笔文坊（公测）

VCPChat 内置的 **AI-native 可编程文档与演示创作系统**现已开启公测。Scriptorium 让人类直接编辑渲染后的真实文档，让 Agent 精确操作同一份 Markdown / HTML / CSS 真源，并通过源码差异、渲染差异、PR 审批、版本文脉与安全审查构成完整的协作闭环。

- **渲染后即时编辑且不牺牲源码**：光标、选区和格式化操作映射回源码字符区间，只替换真正发生变化的片段，保留未触碰源码的原始表达。
- **两类原生工程**：VDOCX 面向 Markdown-first 连续流文稿，VPPTX 面向完整 HTML Scene 演示，均支持资源管理、样式、运行时与独立导出。
- **可编程文档与演示**：支持受控 HTML、LaTeX、Mermaid、SVG、Anime.js、Three.js 以及具备生命周期管理的可编程岛。
- **人类与 Agent 共笔协作**：Agent 通过语义、源码和视觉三种上下文工作，所有写操作进入可审阅 PR，由人类批准后合并，并受到 revision、幂等与并发保护。
- **作品可脱离编辑环境运行**：可导出可独立打开、放映、阅读或打印的 HTML 成品，最终受众无需安装 VCPChat 或 Scriptorium。
- **完整工程文脉**：单个 `.vdocx` / `.vpptx` 工程自带作者署名、提交记录、双重差异、审批回执、快照和安全回溯能力。

详细设计、命令接口、启动方式与当前边界请参阅 [`ScriptoriumModules/README.md`](ScriptoriumModules/README.md)。

---

### 🕸️ VCP Loom — Web 原生 Agent 应用系统 (2026.08.09)

VCP Loom 将任意现代 Web 运行时封装为可长期使用的独立 LoomAPP，让 Agent 能够把网页应用、在线服务、生产力工具和硬件控制台纳入自己的能力体系。

- **语义化网页理解**：VCP Agent WebCore 将页面转译为经过降噪的树状 Grounded Markdown、交互节点和页面状态，Agent 无需直接解析 HTML、CSS 或 DOM。
- **可靠连续操作**：Agent 可通过语义句柄执行点击、输入、选择、滚动、等待和截图，并以快照与文档代次校验每一步，适应动态网页变化。
- **深层运行时能力**：在受控权限下支持脚本、无障碍树、可信原生输入、网络、存储、调试器与 CDP 等能力。
- **独立会话与可定制环境**：每个 LoomAPP 拥有独立持久会话，并可配置窗口、视口、请求特征、辅助样式和注入脚本。
- **硬件与应用生态**：支持 WebHID、WebUSB、WebSerial、WebBluetooth 等 Web 硬件接口，并可导入导出 Loom 分发包。
- **安全与用户主权**：通过上下文隔离、沙箱、来源校验、敏感信息脱敏和用户授权降低风险，权限与审批由 VCP 引擎统一管理。

> Loom 的目标不是再造浏览器，而是把 Web 变成 Agent 的应用商店、工具箱与硬件总线。

## 主要功能

*   **🪄 高级回复 (VCPChatTarven) — 类 SillyTavern 的轻量预设系统**:
    *   **无历史污染的精确干预**: 通过统一管理和热加载的预设 / 注入规则，在不污染聊天历史的前提下干预每一次 AI 请求，并在单聊与群聊中一致生效。
    *   **极速入口**: 在主聊天界面右键发送按钮即可呼出「高级回复」浮窗，独立开关每条规则；通过规则管理窗口可新建、编辑和删除规则。
    *   **三类注入能力**: 支持在 Agent / 群组系统提示词尾部追加内容、仅向本轮用户消息副本追加临时内容，以及将独立的用户或助手消息按指定深度插入上下文。
    *   **包裹与裸注入**: 默认使用 VCPChat 临时注入标记包裹规则，也可按规则关闭标记并原样发送，以适配要求纯净格式的高级提示词工程。
    *   **精细作用范围**: 每条规则可独立设置为全局、仅单聊或仅群聊，并兼容邀请发言、自然随机、顺序轮发、普通对话和续写等请求路径。
    *   **群聊安全**: 用户消息尾部注入仅作用于本轮 AI 提交副本，绝不写入真实群聊历史，保障群文件、跨端记忆与回放分支的一致性。
*   **🧠 Memo 神经云图管理中心**:
    *   **全局记忆可视化**: 基于浪潮 V7 引擎一键生成全局记忆神经节点云图，实时展示所有 Agent 记忆之间的逻辑能流与关联脉络。
    *   **交互式记忆拓扑**: 可选择一个或多个记忆文档并加亮关联节点，直观理解每份记忆在数据库中的逻辑拓扑关系。
    *   **原生多模态记忆**: 笔记、录音、视频、PDF 等多种内容均可作为神经节点纳入云图。
    *   **神经元批量编辑**: 可基于网络拓扑批量编辑近邻日记、合并相关事件，显著提高记忆整理效率。
    *   **Memo 工作台**: 支持选择多篇日记作为参考撰写新日记，调度 Agent 代笔或自动补齐标签。
*   **🖥️ VCPDesktop — AI 原生桌面**:
    *   **流式桌面推送**: AI 可将任意内容流式推送到 VCP 桌面，实时生成天气、日程、新闻等 Widget，并以约 100ms 延迟感知、创建、编辑和管理它们。
    *   **深度后端集成**: 深度集成 VCP 后端 WebSocket 和 Admin API，支持悬浮通知、系统交互与后端数据自动拉取。
    *   **桌面级持久化**: 可收藏 Agent 发出的桌面级内容和悬浮 HTML，构建天气、音乐、网页、游戏等持久桌面组件。
    *   **VCPCli 深度集成**: 实时同步用户电脑桌面或文件夹中的快捷方式，以真正的桌面方式存在和工作。
    *   **高性能流式渲染**: 依托 VChat 流式渲染引擎与墓碑冻结机制，在复杂动态桌面中保持优异性能。
    *   **全局主题一体化**: 支持分布式主题广播，并允许 Agent 动态修改布局、样式、壁纸和高级动态壁纸。
    *   **次世代信息面板**: AI 可在大屏幕上演示和教学高维复杂内容，构建可交互的信息展示界面。
    *   **完整技术文档**: [VCPDesktop 介绍文档](Desktopmodules/VCPdesktop介绍文档.md) 包含系统架构、文件结构、AI 原生能力、挂件接口与设计哲学。
*   **VCP 服务器集成**: 引擎的核心交互功能依赖于 VCP 服务器。它通过 HTTP(S) 与 VCP 服务器通信，发送用户消息并接收 AI 的响应，支持流式传输以实现实时交互。
...
*   **革命性的实时差分渲染：让对话“活”起来**:
    *   **技术核心**: VCPChat 引入了业界前所未有的“流式聊天历史文件差分渲染器”。当底层聊天记录（无论是本地文件还是数据库）发生任何变动，VCPChat 不会粗暴地刷新整个界面或重构 DOM，而是通过精密的差分算法，将“变动”本身流式地、实时地渲染到前端，实现了对 UI 的“外科手术式”更新，极致流畅且稳定。
    *   **渲染器 v3：从“能流式显示”升级到“能流式组织复杂应用”**:
        *   **绝对增量 MorphDOM 流式管理器**: 新版渲染器基于更高级的竞态仲裁管理，实现了绝对增量式的 MorphDOM 流式更新。其核心目标不是“尽快刷新”，而是**尽可能只追加、只替换需要变化的局部 DOM**，避免全局重绘、整块回灌和大面积刷新，让长对话、长文档、复杂卡片在持续流式生成时依然保持稳定、顺滑。
        *   **任意渲染器交叉渲染与嵌套**: 渲染器 v3 支持多种渲染器在同一气泡内进行稳定的交叉渲染与深层嵌套。例如 `CSS + Markdown`、`Anime.js + Three.js`、`Python + HTML`，乃至表格、图片、脚本、文档片段的多层混合嵌套，均可在流式输出过程中按依赖顺序可靠拼装。对于需要边生成边执行、边展示边补全的复杂内容，这一能力尤其关键。
        *   **墓碑冻结系统（Tombstone Freeze）**: 对所有用户当前不可见区域的 DOM 节点进行有状态冻结，而不是仅做简单隐藏。系统会结合严格的规则树管理，主动释放不可见区域的 CPU / GPU 占用，并对齐时间硬件计时器禁用相关活跃更新源。这使得 VChat 在上千楼层、每层都可能是一个 WebApp 式聊天环境的超长对话中，依然能维持优异的资源控制能力。
        *   **涟漪渐进渲染技术（Ripple Rendering）**: 渲染队列会优先预判并锁定用户当前可视区域，优先完成“用户正在看”的内容，再向外渐进扩散渲染。它不仅显著提升了主观响应速度，也通过可靠的涟漪队列中止与回收机制，在用户频繁切换界面、跳转上下文或打断生成时，避免无效渲染堆积与内存泄漏。
        *   **Pretext / Predom 预计算技术**: 对文本类、多模态类 DOM 节点，系统可以在真正挂载渲染之前，预先推导其尺寸、排版边界与部分 Canvas 占位信息。这样做无需等待完整渲染即可提前完成布局决策，在窗口防抖、复杂卡片排版稳定性、尺寸控制和整体布局效率上都带来了显著提升。
        *   **滑动 AST 窗口流式渲染**: 传统流式渲染往往需要反复计算整段流式 AST；而 VChat 面对三十多个渲染器并行、仲裁、嵌套的复杂场景，这样的成本会迅速变得沉重。渲染器 v3 采用滑动 AST 窗口机制，将流式内容划分为 `stream` 区与 `stable` 区进行分层计算，把稳定区逐步踢出高频 AST 解析路径，只对活跃窗口进行快速计算，从而显著降低 I/O 消耗、CPU 压力与 RAM 占用。
...
*   **聊天历史/用户数据管理器 (VchatManager)**:
    ...
    *   新增一个独立的、基于 Electron 的可视化管理工具 (`VchatManager/`)，用于查看和编辑引擎 `AppData` 中的用户数据。
...
*   **笔记模块**:
    *   独立的笔记管理窗口，方便记录和整理信息。
    *   新增迷你便签窗口，可通过全局快捷键 `Win + Alt + Z` 快速呼出，便于随手记录为 Markdown 笔记。
 ...
 *   **VCP 分布式服务器兼容**:
    ...
    *   启用后，允许 VCP 后端调用并利用此引擎节点（前端设备）的算力来执行任务。
    *   支持 VCP 后端调用在引擎本地加载的 VCP 插件，扩展了工具执行的能力。
 ...
 *   **音乐播放器与控制**:
    *   内置一个功能完善的音乐播放器，支持播放本地音乐文件，支持 WASAPI 光纤同轴输出。
    ...
    *   **Agent 音乐控制**: 允许 AI Agent 通过 VCP 服务器调用引擎的音乐控制功能，实现 AI 驱动的音乐播放和管理。此功能可在“全局设置”中开启或关闭。
...
*   **桌面系统感知与语义化操控 (Desktop Awareness & Semantic Operation)**: 这是一个史诗级的更新，赋予了 AI 深度感知并直接操控物理桌面系统的能力。
    *   **全动态桌面感知**: AI 可以实时、动态地感知当前系统运行的所有窗口、进程以及它们正在执行的具体内容。最重要的是，这一过程几乎**不占用 AI 的上下文 Token**，实现了极低成本的全局环境感知。
    * **语义级电脑操作**: 突破了传统的视觉识别限制。AI 现在可以无需“看见”窗口（虽然仍支持截图和录屏辅助），直接通过语义指令进行高阶操作。
        * **全能基础动作**: 现已全面支持**左击、右击、长按、连续点击、键盘输入**等所有基础交互功能。
        * **复杂串行指令**: 支持将多个动作组合成链式任务。例如：“滚动到浏览器窗口，点击搜索框，输入‘VCPChat’并回车发送”。
        * **指令示例**: “打开雷电模拟器里的 FGO 狗粮本，使用 5 号队伍开始战斗！” 或 “在 Dota 2 中释放潮汐猎人的大招！”
        * **极简交互**: AI 只需输出这一行语义指令即可完成复杂操作，无需繁琐的坐标点击或像素识别。
    * **连续操作预输入**: 支持对一系列复杂动作进行预设和连续输入，让 AI 能够像熟练的操作员一样流畅地完成链式任务。
...
*   **语音朗读 (TTS)**:
    ...
    *   **下载与使用**：[TTS 引擎下载链接](https://modelscope.cn/models/aihobbyist/GPT-SoVITS-Inference/files)，20-40的N卡下载124版本，50的N卡下载128版本。使用`gsvi.bat`启动 TTS 引擎。
    *   **下载模型**：[下载模型](https://www.modelscope.cn/models/aihobbyist/GPT-SoVITS_Model_Collection/files) ,将模型下载到`/models/v2proplus`文件夹。
    *   **优化输出接口**：VCPChat 使用自创的流式剪枝算法以提高流式输出中 TTS 渲染的延迟降低卡顿(该算法会将音频合成速度提升 600%，实现语音输入和输出延迟都降低至毫秒级)，因此需要 Sovits 输出引擎兼容。请用 VCPChat 仓库源码中提供的魔改版文件来替换原始项目的核心渲染程序代码。将 VCPChat 源码目录中的 sovitstest/GSVI.py 替换 Sovits 目录中的 gsvi_server/GSVI.py；将 sovitstest/my_infer.py 替换 Sovits 目录中的 tools/my_infer.py。
    *   **业界首创的双语混合朗读引擎**:
        *   VCPChat 实现了一套强大的、基于正则表达式的文本切片算法，允许实现**无缝的中日、中英等多语言混合朗读**。
        *   **如何使用**: 在 Agent 设置中，您可以分别设置“主语言模型”和“副语言模型”，并为它们指定不同的正则表达式。
        *   **应用场景**: 例如，您可以将一个中文模型设为主语言，一个日文模型设为副语言，并为副语言设置正则 `\[(.*?)\]`。这样，当 AI 说出 `你好，[[こんにちは]]，今天天气不错` 时，引擎会自动用中文模型朗读“你好，今天天气不错”，并用日文模型朗读“こんにちは”，整个过程流畅自然，真正实现了“AI教你说外语”的场景。
## 技术栈
...
本系统作为 VCP 生态中的**分布式核心节点**，负责构建 AI 原生的运行环境。通过对前端 IPC 和后端 API 的深度虚拟化封装，它将操作系统底层能力、多媒体引擎与 AI 逻辑彻底融合。它不再仅仅是后端的“眼睛”，而是具备自主感知、实时创造与自我维护能力的**智能操作系统层**。
