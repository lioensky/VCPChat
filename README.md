...
### 🪄 高级回复 (VCPChatTarven) — 类 SillyTavern 的轻量预设系统 (2026.05.17)

VCPChat 引入了一套类 SillyTavern 的轻量「预设 / 注入」系统,让你在不污染聊天历史的前提下,对每一次 AI 请求做精确干预。规则统一管理、热加载、单聊与群聊一致生效。

*   **极速入口**: 在主聊天界面 **右键发送按钮** 即可呼出「高级回复」浮窗,浮窗中以独立开关列出每一条已配置的规则,点开点关即时生效;右上角齿轮则进入完整的「规则管理」模态窗,可新建、编辑、删除任意规则。
*   **三类注入能力**:
    *   **系统提示词注入**: 在 Agent / 群组的 System Prompt **尾部** 追加内容,用于注入设定补丁、行为约束、风格守则等长效背景。
    *   **用户消息注入**: 在 **本轮** 用户消息文本尾部追加内容,仅作用于当次 VCP 提交,**不会写入聊天历史**,适合做"临时口令"、"本次输出格式"等一次性提示。
    *   **上下文消息注入**: 以独立的 `user` 或 `assistant` 消息插入到上下文中,并支持按"深度"精确指定位置(`depth = 0` 插入在末尾,`depth = N` 插入到倒数第 N+1 条之前),可用于伪造历史、构造提示桥、引导思维链等高级玩法。
*   **统一注入包裹(可选裸注入)**: 默认情况下,每条注入会被 `[本信息由VCPChat客户端注入] …… [临时注入结束]` 包裹后再送入上下文,便于模型识别"这是临时注入而非真实历史";每条规则单独提供"使用 VCPChat 包裹标记"开关,关闭后即为 **裸注入**,原样发送,方便接入需要纯净格式的高级用法。
*   **作用范围 (scope)**: 每条规则可独立选择 **全局 / 仅单聊 / 仅群聊** 三种作用范围。同一份规则在群聊邀请发言、自然随机、顺序轮发以及单聊普通对话、续写等所有路径下行为一致,不会出现"群聊里少注入一条"之类的兼容性裂缝。
*   **群聊安全**: 群聊场景下的用户消息尾部注入只追加到本轮 AI 提交的副本上,**绝不污染** 写入文件的真实群聊历史,保障群文件、跨端记忆与回放分支的一致性。

> 简而言之,VCPChatTarven 把 SillyTavern 中"预设 + 注入位"那一层最实用的能力,以 VCP 原生、客户端自管理、单聊/群聊统一适配的方式做成了一个独立模块——既能快速给 Agent 贴一片"可拆卸的临时设定",也足以承载更复杂的提示词工程。

### 🧠 Memo 神经云图管理中心 — 全局记忆可视化革命 (2026.03.19)

VChat 前端的 Memo 管理中心迎来里程碑级升级。基于浪潮 V7 引擎实现了**一键可视化全局记忆神经节点云图**，用户可实时观测所有 Agent 记忆之间的逻辑能流动脉络（基于 V7 的对称性破缺有序能索引）。

*   **交互式记忆拓扑**: 用户可以任意点击单个或多个记忆文档，从而加亮对应的关联记忆节点，直观理解每一份记忆在整个数据库中的逻辑拓扑关系。
*   **原生多模态支持**: 记忆文档支持几乎所有格式——笔记、录音、视频、PDF 等，均可作为神经节点纳入云图。
*   **神经元批量编辑**: 基于神经元网络拓扑，可对多个近邻日记进行批量编辑乃至事件合并，让记忆整理效率呈数量级提升。
*   **全新 Memo 工作台**: 新增 Memo 中心工作台页面，支持选中多个日记作为参考来撰写新日记，允许调度 Agent 代笔或自动 Tag 补齐，为日记整理提供更加便捷直观的界面与可视化体验。

### 🖥️ VCPDesktop — 世界首个 AI 原生桌面 (2026.03.22)

一个真正的、次世代的、革命范式的功能落地了——**VCPDesktop**。该子系统几乎完全依托 VCP 强大的基建和 VChat 卓越的性能框架才得以实现，它彻底打穿了 AI 与 UI 之间的界限。

*   **流式桌面推送**: AI 可使用 `<<<[DESKTOP_PUSH]>>>` 语法，将任意内容流式推送至 VCP 桌面，流式生成实时 Widget 插件。从天气、日程到新闻，AI 可以无感地创建、感知、编辑和管理这些悬浮 Widget，做到真正意义上 **100ms 延迟级别的言出法随**。
*   **深度后端集成**: VCPDesktop 已深度集成后端 WebSocket 和 Admin API，拥有丰富的悬浮通知、系统交互、后端数据自动拉取等功能。
*   **桌面级持久化**: 用户可收藏任何 Agent 发出的桌面级 div 气泡或悬浮 HTML，构建持久化的桌面组件——包括但不限于真天气挂件、真音乐播放器、真网页挂件，甚至桌面小游戏。
*   **VCPCli 深度集成**: 自动获取用户 PC 桌面或文件夹进行实时快捷方式同步，以一个真正的桌面的方式存在和工作。
*   **流式渲染引擎**: 依赖 VChat 的流式渲染引擎，可流式渲染任何内容，自带墓碑冻结机制，拥有远超常规 Electron 软件的性能体验。
*   **全局主题一体化**: 支持 VCP 全局分布式主题广播器，享有全局一体化的 UI/UX 风格，同时允许 Agent 动态编辑桌面的布局、CSS 样式、壁纸及动态高级壁纸。
*   **信息展示革命**: 在巨型屏幕上，AI 可以轻松演示和教学任意复杂维度的信息内容，构成次世代级别的信息展示和交互面板，彻底颠覆桌面交互的概念。

> 📖 **VCPDesktop 完整技术文档**：[VCPdesktop 介绍文档](Desktopmodules/VCPdesktop介绍文档.md) — 包含系统架构、文件结构、AI 原生能力详解、挂件 API 参考及设计哲学。
