// modules/lyrics/qrcDecrypt.js
// QQ Music QRC lyric decryption module (TripleDES + zlib)
// Ported from Folia qrcDecrypt implementation
...
function tripledes_crypt(data, keySchedule) {
  let temp = data;
  for (let i = 0; i < 3; i++) {
 ...
 * Supports Hex-encoded strings or raw Buffer/Uint8Array.
 * Returns plain text XML-formatted QRC string.
 */
async function qrcDecrypt(encryptedHexOrBytes) {
  let encryptedBytes;
  if (typeof encryptedHexOrBytes === 'string') {
    const hex = encryptedHexOrBytes.trim();
    const matches = hex.match(/.{1,2}/g);
    if (!matches) throw new Error('Invalid hexadecimal lyric string');
    encryptedBytes = new Uint8Array(matches.map(byte => parseInt(byte, 16)));
  } else {
    encryptedBytes = new Uint8Array(encryptedHexOrBytes);
  }
  ...
  return new Promise((resolve, reject) => {
    zlib.inflate(decryptedBytes, (err, result) => {
      if (err) {
        // Try raw inflate without zlib header
        zlib.inflateRaw(decryptedBytes, (errRaw, resultRaw) => {
          if (errRaw) {
            reject(err);
          } else {
            resolve(resultRaw.toString('utf8'));
          }
        });
      } else {
