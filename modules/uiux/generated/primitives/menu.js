...
export function mountMenu(anchor, props, scope) {
    ...
    if (props.portal)
        list.classList.add('vcp-harness-menu-portal');
    if (props.align === 'end')
        list.classList.add('vcp-harness-menu-align-end');
    if (props.side === 'top')
        list.classList.add('vcp-harness-menu-side-top');
    if (props.dense)
        list.classList.add('vcp-harness-menu-dense');
    if (props.compact)
        list.classList.add('vcp-harness-menu-compact');
    if (!props.items.some(entry => !isSeparator(entry) && !isLabel(entry) && Boolean(entry.submenu?.length)))
        list.classList.add('vcp-harness-menu-scrollable');
    ...
    const renderItem = (entry, target, nested = false) => {
        ...
        if (entry.danger)
            button.classList.add('vcp-harness-menu-item-danger');
        if (entry.icon) {
        ...
        const openSubmenu = () => {
            ...
            if (props.compact)
                submenu.classList.add('vcp-harness-menu-compact');
