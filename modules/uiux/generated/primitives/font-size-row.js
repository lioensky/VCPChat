...
export function mountFontSizeRow(host, select, scope) {
    ...
    row.className = 'vcp-harness-font-size-row';
    const text = document.createElement('div');
    text.className = 'vcp-harness-font-size-row-text';
    const title = document.createElement('div');
    title.className = 'vcp-harness-font-size-row-title';
    title.textContent = '字号';
    const description = document.createElement('div');
    description.className = 'vcp-harness-font-size-row-description';
    description.textContent = '调整界面文字大小';
    text.append(title, description);
    const control = document.createElement('div');
    control.className = 'vcp-harness-font-size-row-control';
    const stepper = document.createElement('div');
    stepper.className = 'vcp-harness-font-size-row-stepper';
    stepper.tabIndex = -1;
    const value = document.createElement('span');
    value.className = 'vcp-harness-font-size-row-value';
    const unit = document.createElement('span');
    unit.className = 'vcp-harness-font-size-row-unit';
    unit.textContent = 'px';
    const arrows = document.createElement('span');
    arrows.className = 'vcp-harness-font-size-row-arrows';
    const up = document.createElement('button');
    up.type = 'button';
    up.className = 'vcp-harness-font-size-row-arrow';
    up.setAttribute('aria-label', '增大字号');
    up.append(makeChevron(CHEVRON_UP_PATH));
    const down = document.createElement('button');
    down.type = 'button';
    down.className = 'vcp-harness-font-size-row-arrow';
