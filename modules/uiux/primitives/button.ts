...
export function mountButton(button: HTMLButtonElement, props: ButtonProps = {}, scope: UiScope): UiDisposer {
    if (!button || !scope) throw new TypeError('Button requires a native button and scope.');
    ensureStyles();
    ...
    const originalInline: Record<string, [string, string]> = Object.fromEntries(['gap', 'height', 'padding', 'border-radius', 'font-size', 'line-height']
        .map(property => [property, [button.style.getPropertyValue(property), button.style.getPropertyPriority(property)]]));
    const variant = props.variant ?? 'ghost';
    const size = props.size ?? 'md';
    if (originalType === null) button.type = 'button';
    ...
    return scope.own(() => {
        icon?.remove();
        if (originalDisplay) button.style.setProperty('display', originalDisplay, originalDisplayPriority);
        ...
        if (originalType === null) button.removeAttribute('type'); else button.setAttribute('type', originalType);
        if (originalClass === null) button.removeAttribute('class'); else button.setAttribute('class', originalClass);
