...
function makeChevron() {
    ...
    node.classList.add('vcp-uiux-language-row-chevron'); node.setAttribute('viewBox', '0 0 14 14'); node.setAttribute('fill', 'none'); node.setAttribute('aria-hidden', 'true'); node.setAttribute('focusable', 'false');
...
export function mountLanguageRow(host: HTMLElement, props: LanguageRowProps, scope: UiScope): LanguageRowController {
    if (!host || !props?.options || !props?.onSelect || !scope) throw new TypeError('LanguageRow requires a host, options, onSelect and scope.');
    ensureStyles();
    const rowScope = scope.child('uiux-language-row');
    const originalChildren = Array.from(host.childNodes);
    const row = document.createElement('div'); row.className = 'vcp-uiux-language-row';
    const text = document.createElement('div'); text.className = 'vcp-uiux-language-row-text';
    const title = document.createElement('div'); title.className = 'vcp-uiux-language-row-title'; title.textContent = props.title ?? 'Language'; text.append(title);
    if (props.description) { const description = document.createElement('div'); description.className = 'vcp-uiux-language-row-description'; description.textContent = props.description; text.append(description); }
    const trigger = document.createElement('button'); trigger.type = 'button'; trigger.className = 'vcp-uiux-language-row-selector'; const label = document.createTextNode(''); trigger.append(label, makeChevron()); row.append(text, trigger); host.append(row);
    ...
    const dispose = scope.own(async () => {
        ...
        await rebuildQueue;
        row.remove(); host.replaceChildren(...originalChildren);
