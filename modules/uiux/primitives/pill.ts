...
export function mountPill(host: HTMLElement, props: PillProps = {}, scope: UiScope): UiDisposer {
    if (!host || !scope) throw new TypeError('Pill requires a host and scope.');
    ensureStyles();
    ...
    if (interactive) host.classList.add('interactive');
    if (props.active) host.classList.add('active');
    if (isButton && originalType === null) (host as HTMLButtonElement).type = 'button';
    if (originalHandler) scope.listen(host, 'click', originalHandler as EventListener);
    return scope.own(() => {
        if (originalClass === null) host.removeAttribute('class'); else host.setAttribute('class', originalClass);
        if (isButton) {
            if (originalType === null) host.removeAttribute('type'); else host.setAttribute('type', originalType);
        }
