...
export function mountChoice(root: HTMLElement, scope: UiScope): UiDisposer {
    ...
    const sync = () => { const checked = labels.map(label => label.querySelector('input[type="radio"]') as HTMLInputElement | null).find(input => input?.checked); if (checked) root.dataset.value = checked.value; };
