...
export function mountOnboardingSurface(props: OnboardingSurfaceProps, scope: UiScope): OnboardingSurfaceController {
    ...
    ensureStyles();
    const overlay = document.createElement('div'); overlay.className = 'vcp-uiux-onboarding-overlay'; overlay.setAttribute('role', 'presentation');
    const mask = document.createElement('div'); mask.className = 'vcp-uiux-onboarding-mask'; mask.setAttribute('aria-hidden', 'true');
    const stage = document.createElement('div'); stage.className = 'vcp-uiux-onboarding-stage'; overlay.append(mask, stage);
