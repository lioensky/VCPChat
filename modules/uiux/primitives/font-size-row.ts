...
export function mountFontSizeRow(host: HTMLElement, select: HTMLSelectElement, scope: UiScope): FontSizeRowController {
    ...
    const row = document.createElement('div'); row.className = 'vcp-uiux-font-size-row';
    const text = document.createElement('div'); text.className = 'vcp-uiux-font-size-row-text';
    const title = document.createElement('div'); title.className = 'vcp-uiux-font-size-row-title'; title.textContent = '字号';
    const description = document.createElement('div'); description.className = 'vcp-uiux-font-size-row-description'; description.textContent = '调整界面文字大小';
    text.append(title, description);
    const control = document.createElement('div'); control.className = 'vcp-uiux-font-size-row-control';
    const stepper = document.createElement('div'); stepper.className = 'vcp-uiux-font-size-row-stepper'; stepper.tabIndex = -1;
    const value = document.createElement('span'); value.className = 'vcp-uiux-font-size-row-value';
    const unit = document.createElement('span'); unit.className = 'vcp-uiux-font-size-row-unit'; unit.textContent = 'px';
    const arrows = document.createElement('span'); arrows.className = 'vcp-uiux-font-size-row-arrows';
    const up = document.createElement('button'); up.type = 'button'; up.className = 'vcp-uiux-font-size-row-arrow'; up.setAttribute('aria-label', '增大字号'); up.append(makeChevron(CHEVRON_UP_PATH));
    const down = document.createElement('button'); down.type = 'button'; down.className = 'vcp-uiux-font-size-row-arrow'; down.setAttribute('aria-label', '减小字号'); down.append(makeChevron(CHEVRON_DOWN_PATH));
