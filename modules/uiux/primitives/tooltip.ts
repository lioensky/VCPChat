...
export function mountTooltip(anchor: HTMLElement, props: TooltipProps, scope: UiScope): TooltipController {
    if (!anchor?.parentNode || !props?.label || !scope) throw new TypeError('Tooltip requires a connected anchor, label and scope.');
    ensureStyles();
    ...
    const place = () => {
        ...
        let placement: TooltipSide = requestedSide;
        ...
        if (hasSize && measured.right > window.innerWidth - EDGE_MARGIN) dx = window.innerWidth - EDGE_MARGIN - measured.right;
        if (hasSize && measured.left + dx < EDGE_MARGIN) dx = EDGE_MARGIN - measured.left;
    ...
    tooltipScope.listen(anchor, 'blur', () => {
        ...
        if (!triggers.hover && !triggers.focus) hide();
    });
