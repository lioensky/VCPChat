...
export function mountSelect(select: HTMLSelectElement, props: SelectProps = {}, scope: UiScope): UiDisposer {
    ...
    const wrap = document.createElement('span'); wrap.className = 'vcp-uiux-select';
    wrap.style.width = '100%';
    const trigger = document.createElement('button'); trigger.type = 'button'; trigger.className = 'vcp-uiux-select-trigger';
    trigger.setAttribute('aria-haspopup', 'menu'); trigger.setAttribute('aria-expanded', 'false');
    if (props.label) trigger.setAttribute('aria-label', props.label);
    const menu = document.createElement('div'); menu.className = 'vcp-uiux-menu-list vcp-uiux-primitive-menu vcp-uiux-menu-scrollable'; menu.setAttribute('role', 'menu'); menu.hidden = true;
    const viewport = document.createElement('div'); viewport.className = 'vcp-uiux-menu-viewport'; viewport.setAttribute('role', 'presentation'); menu.append(viewport);
    ...
    const sync = () => {
        ...
        Array.from(menu.querySelectorAll<HTMLElement>('[role="menuitem"]')).forEach((item, index) => {
            const active = index === select.selectedIndex; item.dataset.selected = String(active); item.classList.toggle('vcp-uiux-menu-item-selected', active); item.tabIndex = active ? 0 : -1; const check = item.querySelector<HTMLElement>('.vcp-uiux-menu-item-check'); if (active && !check) { const marker = document.createElementNS('http://www.w3.org/2000/svg', 'svg'); marker.classList.add('vcp-uiux-menu-item-check'); marker.setAttribute('width', '16'); marker.setAttribute('height', '16'); marker.setAttribute('viewBox', '0 0 16 16'); marker.setAttribute('fill', 'none'); marker.setAttribute('focusable', 'false'); marker.innerHTML = '<path d="M15.0498 3.92579L8.49512 12.3818C8.25774 12.6881 8.04517 12.9645 7.84668 13.1689C7.63957 13.3823 7.38732 13.5841 7.04492 13.6719C6.86373 13.7183 6.6757 13.7346 6.48926 13.7197C6.13666 13.6915 5.8528 13.5355 5.6123 13.3604C5.38201 13.1926 5.12573 12.9567 4.83984 12.6953L1.03125 9.21289L1.96875 8.1875L5.77734 11.6699C6.08684 11.9529 6.27773 12.1249 6.43066 12.2363C6.50183 12.2882 6.54699 12.3135 6.57324 12.3252C6.58525 12.3305 6.59269 12.3322 6.5957 12.333C6.59802 12.3336 6.59961 12.334 6.59961 12.334C6.63317 12.3367 6.66758 12.3335 6.7002 12.3252C6.7002 12.3252 6.70211 12.3251 6.7041 12.3242C6.70698 12.3229 6.71348 12.319 6.72461 12.3115C6.74849 12.2956 6.78843 12.2642 6.84961 12.2012C6.98138 12.0654 7.13957 11.8628 7.39648 11.5313L13.9502 3.07422L15.0498 3.92579Z" fill="currentColor"/>'; item.append(marker); } else if (!active && check) check.remove();
        });
    };
    ...
    const placePortal = () => {
        if (!props.portal || menu.hidden || trigger.getAttribute('aria-expanded') !== 'true') return;
        const anchor = trigger.getBoundingClientRect();
        const margin = 12; const width = menu.offsetWidth || anchor.width; const height = menu.offsetHeight;
    ...
    Array.from(select.options).forEach((option, index) => {
        ...
        const itemWrap = document.createElement('div'); itemWrap.className = 'vcp-uiux-menu-item-wrap';
        const item = document.createElement('button'); item.type = 'button'; item.className = 'vcp-uiux-menu-item'; item.setAttribute('role', 'menuitem'); item.disabled = option.disabled; const label = document.createElement('span'); label.className = 'vcp-uiux-menu-item-label'; const description = option.dataset.description?.trim(); if (description) { const content = document.createElement('span'); content.className = 'vcp-uiux-menu-item-content'; const name = document.createElement('span'); name.className = 'vcp-uiux-menu-item-name'; name.textContent = option.textContent?.trim() || ''; const detail = document.createElement('span'); detail.className = 'vcp-uiux-menu-item-description'; detail.textContent = description; content.append(name, detail); label.append(content); } else label.textContent = option.textContent?.trim() || ''; item.append(label); itemWrap.append(item);
    ...
    scope.listen(window, 'scroll', placePortal, { capture: true }); scope.listen(window, 'resize', placePortal);
    ...
    return scope.own(() => {
        ...
        close(false); if (originalTabIndex === null) select.removeAttribute('tabindex'); else select.setAttribute('tabindex', originalTabIndex); if (originalAriaHidden === null) select.removeAttribute('aria-hidden'); else select.setAttribute('aria-hidden', originalAriaHidden); select.classList.remove('vcp-uiux-select-native'); wrap.replaceWith(select); if (ownsFocus && previousActive && typeof (previousActive as HTMLElement).focus === 'function' && document.contains(previousActive)) (previousActive as HTMLElement).focus();
