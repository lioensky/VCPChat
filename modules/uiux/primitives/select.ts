...
export function mountSelect(select: HTMLSelectElement, props: SelectProps = {}, scope: UiScope): UiDisposer {
    ...
    Array.from(select.options).forEach((option, index) => {
        ...
        const item = document.createElement('button'); item.type = 'button'; item.className = 'vcp-harness-menu-item'; item.setAttribute('role', 'menuitem'); item.disabled = option.disabled; const label = document.createElement('span'); label.className = 'vcp-harness-menu-item-label'; const description = option.dataset.description?.trim(); if (description) { const content = document.createElement('span'); content.className = 'vcp-harness-menu-item-content'; const name = document.createElement('span'); name.className = 'vcp-harness-menu-item-name'; name.textContent = option.textContent?.trim() || ''; const detail = document.createElement('span'); detail.className = 'vcp-harness-menu-item-description'; detail.textContent = description; content.append(name, detail); label.append(content); } else label.textContent = option.textContent?.trim() || ''; item.append(label); itemWrap.append(item);
