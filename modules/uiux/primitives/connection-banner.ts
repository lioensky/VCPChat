...
export function mountConnectionBanner(host: HTMLElement, props: ConnectionBannerProps, scope: UiScope): ConnectionBannerController {
    ...
    const render = () => {
        ...
        if (!banner) { banner = document.createElement('div'); banner.className = 'vcp-uiux-connection-banner banner'; banner.setAttribute('role', 'status'); banner.setAttribute('aria-live', 'polite'); host.replaceChildren(banner); }
        banner.textContent = label;
    };
    render();
    const dispose = scope.own(() => { banner = null; if (originalClass === null) host.removeAttribute('class'); else host.setAttribute('class', originalClass); host.replaceChildren(...originalNodes); }, 'uiux-connection-banner', 'ui-primitive');
