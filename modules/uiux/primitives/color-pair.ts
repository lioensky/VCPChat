...
export function mountColorPair(color: HTMLInputElement, text: HTMLInputElement, scope: UiScope, props: ColorPairProps = {}): UiDisposer {
    ...
    syncText();
    return scope.own(() => { if (color.parentNode === wrap) parent.insertBefore(color, wrap); if (text.parentNode === wrap) parent.append(text); wrap.remove(); }, 'uiux-color-pair', 'ui-primitive');
}
