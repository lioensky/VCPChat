...
export function mountField(root: HTMLElement, props: FieldProps, scope: UiScope): UiDisposer {
    ...
    if (description) { description.className = 'vcp-uiux-field-description'; description.textContent = props.description ?? ''; }
    const error = props.error ? document.createElement('p') : null;
    if (error) { error.className = 'vcp-uiux-field-error'; error.textContent = props.error ?? ''; props.control.classList.replace('vcp-uiux-field-input', 'vcp-uiux-field-input-invalid'); props.control.setAttribute('aria-invalid', 'true'); }
    ...
    return scope.own(() => {
        head.remove(); if (!existingLabel) label.remove(); else { delete existingLabel.dataset.vcpFieldLabel; existingLabel.classList.remove('vcp-uiux-field-label'); }
