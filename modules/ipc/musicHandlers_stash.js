...
function initialize(options) {
    ...
    const registerIpcHandlers = () => {
        ...
        ipcMain.handle('webdav-add-server', (event, config) => {
            const id = webdavManager.addServer(config);
            return { ok: true, id };
        });

        ipcMain.handle('webdav-remove-server', (event, serverId) => {
        ...
        ipcMain.handle('webdav-test-connection', async (event, serverId) => {
            return await webdavManager.testConnection(serverId);
        });

        ipcMain.handle('webdav-list-directory', async (event, { serverId, remotePath }) => {
            return await webdavManager.listDirectory(serverId, remotePath);
        });

        ipcMain.handle('webdav-scan-audio', async (event, { serverId, remotePath }) => {
            const results = await webdavManager.scanAudioFiles(serverId, remotePath, (count) => {
                if (musicWindow && !musicWindow.isDestroyed()) {
        ...
        ipcMain.handle('webdav-get-file-url', (event, { serverId, remotePath }) => {
            return webdavManager.getFileUrl(serverId, remotePath);
        });

        // Load a WebDAV file: get the HTTP URL and pass it to the Rust engine
        ipcMain.handle('webdav-load-track', async (event, { serverId, remotePath, trackMeta }) => {
            const url = webdavManager.getFileUrl(serverId, remotePath);
            if (!url) return { status: 'error', message: 'Could not build URL for track' };
            currentSongInfo = {
                title: (trackMeta && trackMeta.title) || remotePath.split('/').pop(),
                artist: (trackMeta && trackMeta.artist) || '未知艺术家',
                album: (trackMeta && trackMeta.album) || '未知专辑',
            };
            return audioEngineApi('/load', 'POST', { path: url });
        });
    };
