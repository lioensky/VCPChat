...
function setupWebDav(app) {
    app.openWebDavModal = async () => {
        ...
        app.webdavServers = await window.electron.invoke('webdav-list-servers') || [];
    ...
    app.renderWebDavServerList = () => {
        ...
        app.webdavServers.forEach(server => {
            ...
            div.querySelector('.webdav-server-name').addEventListener('click', () => {
                app.activeWebDavServer = server; app.webdavCurrentPath = '/';
                app.webdavScannedTracks = []; app.webdavScanBtn.disabled = false;
                app.webdavImportBtn.disabled = true; app.renderWebDavServerList();
                app.browseWebDav('/');
            });
            div.querySelector('.webdav-server-remove').addEventListener('click', async (e) => {
                ...
                if (app.activeWebDavServer && app.activeWebDavServer.id === server.id) {
                    ...
                    app.webdavBreadcrumb.innerHTML = ''; app.webdavScanBtn.disabled = true;
    ...
    app.browseWebDav = async (dirPath) => {
        if (!app.activeWebDavServer) return;
        ...
        const result = await window.electron.invoke('webdav-list-directory', {
            serverId: app.activeWebDavServer.id, url: app.activeWebDavServer.url, path: dirPath
        });
    ...
    app.renderWebDavBreadcrumb = (dirPath) => {
        ...
        parts.forEach(part => {
            ...
            const span = document.createElement('span'); span.className = 'webdav-crumb';
    ...
    app.playWebDavTrack = async (entry) => {
        if (!app.activeWebDavServer || !window.electron) return;
        const track = { title: entry.name.replace(/\.[^.]+$/, ''), artist: '', album: '', path: entry.url, isRemote: true, serverId: app.activeWebDavServer.id };
        ...
        await window.electron.invoke('webdav-load-track', {
            url: entry.url, serverId: app.activeWebDavServer.id, trackMeta: { title: track.title, artist: track.artist, album: track.album }
        });
    ...
    app.setupWebDavHandlers = () => {
        ...
        app.webdavScanBtn?.addEventListener('click', async () => {
            if (!app.activeWebDavServer) return;
            app.webdavScanBtn.disabled = true; app.webdavScanBtn.textContent = '扫描中...';
            window.electron.on('webdav-scan-progress', ({ count }) => { app.webdavScanBtn.textContent = `已找到 ${count} 首...`; });
            try {
                const result = await window.electron.invoke('webdav-scan-audio', {
                    serverId: app.activeWebDavServer.id, url: app.activeWebDavServer.url
                });
                if (result && result.status === 'success') {
            ...
            app.webdavScanBtn.textContent = '扫描全部音频'; app.webdavScanBtn.disabled = false;
        });
        app.webdavImportBtn?.addEventListener('click', () => {
            if (app.webdavScannedTracks.length === 0) return;
            const newTracks = app.webdavScannedTracks.map(t => ({ ...t, serverId: app.activeWebDavServer.id, isRemote: true }))
                .filter(rt => !app.playlist.some(p => (p.title || p.name || '').toLowerCase() === (rt.title || rt.name || '').toLowerCase()));
            app.playlist.push(...newTracks);
            app.currentFilteredTracks = null; app.renderPlaylist(null);
            window.electron.send('save-music-playlist', app.playlist);
