...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  const editorContextSelectAll = document.getElementById(
    "editor-context-select-all"
  );
  const helpBtn = document.getElementById("helpBtn");
  const noteScopeFilter = document.getElementById("noteScopeFilter");
  const noteTagFilter = document.getElementById("noteTagFilter");
  ...
  const markdownToolbarButtons = Array.from(
  ...
  function updatePreviewToggleState(isPreviewCollapsed) {
    ...
    previewToggleBtn.setAttribute(
      "aria-label",
      isPreviewCollapsed ? "打开预览区" : "关闭预览区"
    );
  ...
  function setViewMode(mode) {
    const normalizedMode = ["editor", "split", "preview"].includes(mode)
      ? mode
      : "split";
    document.body.dataset.noteViewMode = normalizedMode;
    localStorage.setItem("notesViewMode", normalizedMode);
    ...
    if (normalizedMode === "split") {
      updatePreviewToggleState(false);
    }

    requestAnimationFrame(() => {
      if (window.pretextBridge?.isReady()) {
        window.pretextBridge.recalculateAll(window.innerWidth);
      }
    });
  }
  ...
  function showLoadingOverlay(message) {
    ...
    if (!overlay) {
      ...
      overlay.style.cssText =
        "position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.6); display:flex; justify-content:center; align-items:center; z-index:10000; color:white; flex-direction:column;";
  ...
  function applyTheme(theme) {
    ...
    const highlightThemeStyle = document.getElementById(
  ...
  function addParserBreakerBetweenDivAndCode(text) {
    if (typeof text !== "string") return text;
    const regex =
      /(<\/div>)\s*(```(?=[\s\S]*?(?:<<<\[TOOL_REQUEST\]>>>|<<<DailyNoteStart>>>)))/g;
  ...
  function ensureSpecialBlockFenced(text, startTag, endTag) {
    ...
    const regex = new RegExp(
      `(\`\`\`[\\s\\S]*?${startTag}[\\s\\S]*?${endTag}[\\s\\S]*?\`\`\`)|(${startTag}[\\s\\S]*?${endTag})`,
      "g"
    );
  ...
  function ensureHtmlFenced(text) {
    ...
    while (true) {
      const startIndex = text
        .toLowerCase()
        .indexOf(doctypeTag.toLowerCase(), lastIndex);
      const textSegment = text.substring(
        lastIndex,
      ...
      const endIndex = text
        .toLowerCase()
        .indexOf("</html>", startIndex + doctypeTag.length);
      if (endIndex === -1) {
    ...
    for (const line of lines) {
      ...
      if (mathLines) {
        if (/^(?:\\|\\\\)\]$/.test(trimmed)) {
          ...
          output.push("");
          output.push(
            `<div class="vcp-math-placeholder" data-vcp-math-id="${id}"></div>`
          );
          output.push("");
          mathLines = null;
  ...
  function renderExtractedLatexBlocks(container, blocks) {
    ...
    container
      .querySelectorAll(".vcp-math-placeholder[data-vcp-math-id]")
      .forEach((node) => {
  ...
  function preprocessFullContent(text) {
    ...
    processed = ensureSpecialBlockFenced(
      processed,
      "<<<[TOOL_REQUEST]>>>",
      "<<<[END_TOOL_REQUEST]>>>"
    );
  ...
  function renderMarkdown(markdown) {
    ...
    const sanitizedMarkdown = processedMarkdown.replace(
      /!\[(.*?)\]\(file:\/\/([^)]+)\)/g,
      (match, alt, url) => {
    ...
    if (
      ...
      window.pretextBridge.isReady() &&
      activeNoteId
    ) {
      const previewWidth = previewContentDiv.offsetWidth || 500;
      window.pretextBridge.estimateHeight(
        "note-" + activeNoteId,
        markdown,
        "note",
        previewWidth
  ...
  const MERMAID_CONTENT_PATTERN =
    /^\s*(?:graph|flowchart|sequenceDiagram|classDiagram|stateDiagram(?:-v2)?|erDiagram|journey|gantt|pie|gitGraph|mindmap|timeline|requirementDiagram|quadrantChart|C4Context|C4Container|C4Component|C4Dynamic|C4Deployment|xychart-beta|sankey-beta|block-beta)\b/;
  ...
  async function renderMermaidNodes(nodes, renderTokenId) {
    ...
    try {
      ...
      nodes.forEach((el) => {
        ...
        el.innerHTML = `<div class="mermaid-error">Mermaid 渲染错误: ${escapeHtmlForMermaid(
          error.message || String(error)
        )}</div><pre>${escapeHtmlForMermaid(originalCode)}</pre>`;
      });
    }
  ...
  function matchesNoteFilters(item, query) {
    if (item.type !== "note") return false;
    const tags = getTagsForNote(item);
    const normalizedQuery = query.trim().toLowerCase();
    const matchesQuery =
      !normalizedQuery ||
      [item.title, item.content, tags.join(" ")].some((value) =>
        String(value || "")
          .toLowerCase()
          .includes(normalizedQuery)
      );
    const matchesTag =
      !noteTagFilter.value || tags.includes(noteTagFilter.value);
  ...
  function filterTree(tree, query) {
    ...
    for (const item of tree) {
      if (item.type === "note") {
        if (matchesNoteFilters(item, query)) result.push(item);
      } else if (item.type === "folder") {
        const children = filterTree(item.children || [], query);
        if (children.length > 0 || (!query && !noteTagFilter.value)) {
  ...
  function sortTree(tree, sortMode) {
    if (sortMode === "custom") return tree;
    const sorted = tree.map((item) =>
      item.type === "folder"
        ? { ...item, children: sortTree(item.children || [], sortMode) }
        : item
    );
  ...
  function getAvailableTags() {
    ...
    const collect = (tree) =>
      tree.forEach((item) => {
        if (item.type === "note")
          getTagsForNote(item).forEach((tag) => tags.add(tag));
        if (item.type === "folder") collect(item.children || []);
      });
    collect(getCombinedTree());
  ...
  function refreshTagFilterOptions() {
    ...
    noteTagFilter.replaceChildren(new Option("全部标签", ""));
    tags.forEach((tag) => noteTagFilter.add(new Option(`#${tag}`, tag)));
    noteTagFilter.value = tags.includes(selectedTag) ? selectedTag : "";
  }
  ...
  function renderTree() {
    ...
    const sortMode = noteSortSelect.value;
    const scope = noteScopeFilter.value;
    const groups = [
      { key: "local", title: "本地笔记", tree: localNoteTree },
      { key: "network", title: "网络笔记", tree: networkNoteTree },
    ].filter((group) => scope === "all" || scope === group.key);

    const fragment = document.createDocumentFragment();
    groups.forEach((group) => {
      const filteredTree = sortTree(filterTree(group.tree, query), sortMode);
      const section = document.createElement("li");
      section.className = "note-source-group";
      section.dataset.source = group.key;

      const heading = document.createElement("div");
      heading.className = "note-source-heading";
      heading.textContent = group.title;
      section.appendChild(heading);

      if (filteredTree.length > 0) {
        const groupList = document.createElement("ul");
        groupList.className = "note-source-content";
        filteredTree.forEach((item) =>
          groupList.appendChild(createTreeElement(item))
        );
        ...
        empty.textContent =
          query || noteTagFilter.value ? "没有符合条件的笔记" : "暂无笔记";
    ...
    if (isFolder) {
      ...
      if (item.children && !isCollapsed) {
        item.children.forEach((child) =>
          childrenUl.appendChild(createTreeElement(child))
        );
      }
      ...
      const tagSpans =
        tags.length > 0
          ? `<span class="note-tag-list">${tags
              .map((tag) => `<span class="note-tag">#${tag}</span>`)
              .join("")}</span>`
          : "";
      const timeSpan = `<span class="note-timestamp-display">${new Date(
        item.timestamp
      ...
      li.innerHTML = `${NOTE_ICON} <span class="note-item-details">${nameSpan}${tagSpans}</span> ${timeSpan}`;
  ...
  function updateSelectionVisuals() {
    ...
    const getVisualTarget = (li) => {
      if (!li) return null;
      return li.matches(".note-item")
        ? li
        : li.querySelector(":scope > .folder-header-row");
    };
    ...
    if (activeItemId) {
      const li = noteList.querySelector(
        `li[data-id="${CSS.escape(activeItemId)}"]`
      );
      const target = getVisualTarget(li);
  ...
  function handleItemClick(event, item) {
    ...
    if (event.shiftKey && activeItemId) {
      ...
      const allItems = Array.from(noteList.querySelectorAll("[data-id]")).map(
        (el) => el.dataset.id
      );
  ...
  async function createNewItem(type, ext = ".md", template = null) {
    // Default to .md for backward compatibility if needed
    let parentPath;
    const activeItem = activeItemId
      ? findItemById(getCombinedTree(), activeItemId)
      : null;
    ...
    if (type === "folder") {
      ...
      const folderResult = await api.createNoteFolder({
        parentPath,
        folderName,
      });
      await loadNoteTree();
      if (folderResult?.success) startInlineRename(folderResult.id);
      return;
    }

    const newNote = {
      title: template?.title || "无标题笔记",
    ...
    if (result.success) {
      ...
      await loadNoteTree();
      await selectNote(result.id, result.filePath);
  ...
  async function insertImageFile(file) {
    ...
    reader.onerror = async () =>
      showErrorModal("读取图片失败", "无法读取所选图片，请重试。");
    reader.onload = async (event) => {
      const result = await api.savePastedImageToFile(
        {
      ...
      if (!result?.success || !result.attachment) {
        await showErrorModal("插入图片失败", result?.error || "图片未能保存。");
        return;
      }

      insertTextAtSelection(
        `![${result.attachment.name}](${result.attachment.internalPath})`
      );
      showButtonFeedback(insertImageBtn, "插入图片", "已插入", true, 1200);
    };
    reader.readAsDataURL(file);
  }

  function insertTextAtSelection(text, selectionOffset = 0) {
    const { selectionStart, selectionEnd, value } = noteContentInput;
    const nextContent = `${value.slice(0, selectionStart)}${text}${value.slice(
      selectionEnd
    )}`;
    const caret = selectionStart + text.length + selectionOffset;
    noteContentInput.value = nextContent;
  ...
  function applyMarkdownAction(action) {
    if (!activeNoteId) {
      showInfoModal("请先打开笔记", "请先新建或打开一篇笔记，再使用格式工具。");
      return;
    }

    const selectedText = noteContentInput.value.slice(
    ...
    const actions = {
      ...
      bulletList: { prefix: "- ", suffix: "", placeholder: "列表项目" },
    ...
    const offset = selectedText ? 0 : -(config.suffix.length || 0);
    insertTextAtSelection(insertion, offset);
  }
  ...
  function showNotesHelp() {
    showInfoModal(
      ...
      `
        ...
        <code># 标题</code>　<code>**加粗**</code>　<code>- 列表</code>　<code>[链接](https://)</code><br><br>
        <strong>快捷方式</strong><br>
        <code>Ctrl+S</code> 保存　<code>Ctrl+F</code> 搜索当前笔记　<code>Ctrl+B</code> 加粗
      `
    );
  }
  ...
  function selectEditorSearchMatch(index, options = {}) {
    ...
    const lineHeight =
      parseFloat(getComputedStyle(noteContentInput).lineHeight) || 22;
    const textBeforeMatch = noteContentInput.value.slice(0, matchStart);
    const lineIndex = textBeforeMatch.split("\n").length - 1;
    const targetScrollTop = Math.max(
      0,
      lineIndex * lineHeight - noteContentInput.clientHeight / 2
    );
  ...
  function findEditorMatch(direction = 1) {
    ...
    if (direction >= 0) {
      const nextIndex = editorSearchMatches.findIndex(
        (matchIndex) =>
          matchIndex >= selectionStart + (editorSearchIndex >= 0 ? 1 : 0)
      );
  ...
  async function pasteTextIntoEditor() {
    ...
    try {
      if (navigator.clipboard?.readText) {
        ...
        const { selectionStart, selectionEnd, value } = noteContentInput;
        noteContentInput.value = `${value.slice(
          0,
          selectionStart
  ...
  function updateEditorContextMenuState() {
    const hasSelection =
      noteContentInput.selectionStart !== noteContentInput.selectionEnd;
  ...
  editorContextCut.addEventListener("click", () => {
    if (!editorContextCut.classList.contains("disabled"))
      runEditorCommand("cut");
    hideEditorContextMenu();
  });

  editorContextCopy.addEventListener("click", () => {
    if (!editorContextCopy.classList.contains("disabled"))
  ...
  helpBtn.addEventListener("click", showNotesHelp);
  noteScopeFilter.addEventListener("change", renderTree);
  noteTagFilter.addEventListener("change", renderTree);
  noteSortSelect.addEventListener("change", renderTree);
  noteTemplateSelect.addEventListener("change", async () => {
    const template = NOTE_TEMPLATES[noteTemplateSelect.value];
    noteTemplateSelect.value = "";
    if (template) await createNewItem("note", ".md", template);
  });
  viewModeButtons.forEach((button) => {
  ...
  async function saveCurrentNote(isAutoSave = false) {
    if (!activeNoteId) {
      if (!isAutoSave)
        showButtonFeedback(saveNoteBtn, "保存", "无活动笔记", false);
      return;
    }
  ...
  async function handleListDragStart(e) {
    ...
    e.dataTransfer.setData(
      "application/vnd.vcp-notes.items+json",
      JSON.stringify(dragState.sourceIds)
    );

    // Immediately add dragging class synchronously for snappier visual feedback
    dragState.sourceIds.forEach((selectedId) => {
      const el = noteList.querySelector(
        `li[data-id="${CSS.escape(selectedId)}"]`
      );
  ...
  function updateDragAutoScroll(clientY) {
    ...
    const scrollStep = () => {
      ...
      if (pointerY < listRect.top + threshold) {
        speed = -Math.ceil(
          ((listRect.top + threshold - pointerY) / threshold) * maxSpeed
        );
      } else if (pointerY > listRect.bottom - threshold) {
        speed = Math.ceil(
          ((pointerY - (listRect.bottom - threshold)) / threshold) * maxSpeed
        );
      }
  ...
  function cleanupDragOverVisuals() {
    if (dragState.lastDragOverVisualElement) {
      dragState.lastDragOverVisualElement.classList.remove(
        "drag-over-folder",
  ...
  function isInvalidDropTarget(targetElement) {
    ...
    return dragState.sourceIds.some((sourceId) => {
      const sourceElement = noteList.querySelector(
        `li[data-id="${CSS.escape(sourceId)}"]`
      );
      return (
        sourceElement &&
        sourceElement !== targetElement &&
        sourceElement.contains(targetElement)
      );
    });
  }
  ...
  function getVisibleDropRows() {
    const listRect = noteList.getBoundingClientRect();
    return Array.from(
      noteList.querySelectorAll(".note-item, .folder-header-row")
    )
      .map((row) => {
  ...
  function computeDropIntentFromPoint(clientX, clientY) {
    ...
    if (
      directRow &&
      directItem &&
      !isInvalidDropTarget(directItem) &&
      directItem.dataset.type === "folder"
    ) {
      ...
      if (
        offsetY >= insideThreshold &&
        rect.bottom - clientY >= insideThreshold
      ) {
        return {
  ...
  function applyDropIntentVisuals(intent) {
    ...
    if (
      dragState.lastDragOverVisualElement &&
    ...
    visualElement.classList.toggle(
      "drag-over-target-top",
      dropAction === "before"
    );
    visualElement.classList.toggle(
  ...
  async function handleListDrop(e) {
    ...
    const sourceIds = Array.isArray(dragState.sourceIds)
      ? [...dragState.sourceIds]
      : null;
    ...
    if (dragState.lastPointer) {
      const finalIntent = computeDropIntentFromPoint(
        dragState.lastPointer.clientX,
        dragState.lastPointer.clientY
      );
      if (finalIntent) {
    ...
    const sourcePaths = sourceIds
      .map((id) => findItemById(getCombinedTree(), id)?.path)
      .filter(Boolean);
    ...
    try {
      target = {
        ...
        destPath:
          dropAction === "inside"
            ? targetItem.path
            : await window.electronPath.dirname(targetItem.path),
      };
    } catch (error) {
    ...
    try {
      ...
      if (!result || !result.success) {
        ...
        const message = result.renamedItems
          .map(
            (item) =>
              `"${getFileName(item.oldPath)}" 已重命名为 "${getFileName(
                item.newPath
              )}"`
          )
          .join("<br>");
  ...
  document.addEventListener(
    ...
    (e) => {
      if (
        !e.target.closest("#noteList [data-id]") &&
        !customContextMenu.contains(e.target)
      ) {
        customContextMenu.style.display = "none";
      }
      if (
        !e.target.closest("#noteContent") &&
  ...
  async function handleDirectDelete(isFromContextMenu = true) {
    ...
    const itemsToDelete = Array.from(selectedItems)
      .map((id) => findItemById(getCombinedTree(), id))
      .filter(Boolean);

    // Prevent deleting the protected folder
    const isProtectedFolderSelected = itemsToDelete.some(
      (item) =>
        isCloudItem(item.id) && (item.name || item.title).includes("dailynote")
    );
    ...
    const containsCloudFolder = itemsToDelete.some(
      (item) => item.type === "folder" && isCloudItem(item.id)
    );
      ...
      for (const item of itemsToDelete) {
        ...
        if (result.success) {
          ...
          if (networkNoteTree && networkNoteTree.length > 0) {
            networkNoteTree.forEach((tree) =>
              removeItemById(tree.children, item.id)
            );
          }
        } else {
  ...
  function initEditorPreviewResizer() {
    ...
    const applySplit = (percent) => {
      ...
      editorPreviewResizer.setAttribute(
        "aria-valuenow",
  ...
  async function initializeApp() {
    ...
    const isPreviewCollapsed =
      localStorage.getItem("notesPreviewCollapsed") === "true";
    updatePreviewToggleState(isPreviewCollapsed);
    setViewMode(localStorage.getItem("notesViewMode") || "split");
    ...
    networkNoteTree = Array.isArray(cachedNetworkNotes)
      ? cachedNetworkNotes
      : cachedNetworkNotes
      ? [cachedNetworkNotes]
      : [];
    ...
    api.onNetworkNotesScanned((freshNetworkTree) => {
      // Ensure it's always an array, handling both old object format and null/undefined
      networkNoteTree = Array.isArray(freshNetworkTree)
        ? freshNetworkTree
        : freshNetworkTree
        ? [freshNetworkTree]
        : [];
    ...
    api.onSharedNoteData(async (data) => {
      ...
      const now = new Date();
      const generatedTitleSuffix = `.${String(now.getMilliseconds()).padStart(
        3,
        "0"
      )}`;
      const generatedTitle = `分享笔记 ${now.getFullYear()}-${String(
        now.getMonth() + 1
      ...
      if (result.success) {
        ...
        console.error(
          "Failed to create new note from shared content:",
          result.error
        );
      }
    });
  ...
  noteContentInput.addEventListener("paste", async (event) => {
    ...
    for (const item of items) {
      if (item.type.startsWith("image/")) {
        ...
        await insertImageFile(item.getAsFile());
        return;
      }
    }
  });
