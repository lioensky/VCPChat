...
function createCatalog(ops) {
    return {
        ...
        onRollDice: subscription(ops.subscribe('roll-dice', (_event, notation, options) => [notation, options])),
        ...
        loomListApps: query(() => ops.invoke('loom:list-apps')),
        loomOpenApp: query((appId) => ops.invoke('loom:open-app', appId)),
        loomOpenManager: query(() => ops.invoke('loom:open-manager')),
        onLoomRegistryChanged: subscription(ops.subscribe('loom:registry-changed', (_event, apps) => apps)),
...
exposeRoleApis('desktopAPI', roleApi, compatApi, ops);
