...
> 状态（2026-08-20）：D0、D1 已完成；D2、D3 已具备真实协调器、operation identity、主聊天/独立 Surface/辅助窗口消费者和 Electron 终态证据。StreamProjection 已按 Surface 工厂化，每个 Surface 拥有独立 DOM projection、固定 conversation identity、terminal route 和 quiescent dispose；独立聊天不再读取主窗口 selection/history。D4 已建立 RenderDependencies、SurfaceConversation 和显式 DOM realm/service capability closure。近期增量已加入注入式 viewAuthority、owned animation-frame scheduling、pending operation dispose 等证据，并收紧 MessageRenderer 的 root/style owner；StreamProjection 现在要求 owning root 和显式 view authority，transient message model 已交给 `StreamTransientHistory` provider。D5 正在进行：MainChatSurfaceAdapter 已组合主聊天 stream/operation 生命周期，主窗口发送状态改由 composition root 显式注入，renderer.js 仍保留主题、设置、Electron 和事件 wiring；StreamProjection 仍负责 terminal DOM 投影，但不再拥有 repository 或 history mutation。D6 已删除三个旧全局 facade 与全部 legacy history IPC fallback；其他 renderer legacy methods、operation identity 全面下沉和 ambient DOM 路径仍待收口。D7 未完成。当前证据：Chat Kernel 105/105、UI System 83/83、Electron UI Apps 24/24、主聊天 Electron 24 actions、生命周期压力 3 次预热 + 20 次循环、`guard:next-delta`、`guard:design-subtraction`、`guard:classic-retirement`、`guard:chat-kernel-consumers` 均通过。未满足全部退出条件前不得宣称 renderer 已完成解耦。
