...
- `MainChatSurfaceAdapter` 已组合主聊天 Surface、stream route、bridge、capability provider 与 teardown；终态副作用和错误 DOM 已从 `renderer.js` closure 移入 adapter。`renderer.js` 仍保留主题、设置、Electron 和 ChatManager wiring，D5 尚未完成。

最近验证：`test:chat-kernel` 98/98、`test:ui-system` 81/81、Electron UI apps 24/24、主聊天 24 步、`guard:next-delta`、`guard:chat-kernel-consumers`、`guard:classic-retirement` 均通过。主聊天真实序列另覆盖独立 Surface 在主话题切换后的持续投影、主/独立并发隔离取消、Flowlock 非流式唯一 durable terminal、VoiceChat/Rust Assistant 成功终态、pending close cancellation、无 thinking 残留和最终新话题持久化。此前一次运行在独立 Surface 终态投影中暴露已删除 `messageRoot` 残留引用，已修复并补充直接终态测试；一次 smoke 在固定 conversation ref 的 setter 合同上暴露初始化错误，已修复并复跑通过。D7 仍需完整生命周期压力、辅助窗口并发/恢复和 renderer 历史路径收口证据。
